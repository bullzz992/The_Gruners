...
public class Detect extends Activity implements OnTaskCompleted, GPSCallback{
	...
	public static final String PACKAGE_NAME = "up.csir.proj.sanpd";
...
//		gpsManager = null;
	...
	public class PlateView extends View implements Camera.PreviewCallback,
		...
		public PlateView(Detect context) throws IOException {
			super(context);
		}
		...
		//Check duplicate numberplates in history
		
		public boolean checkDuplicates(String num)
		{
			BufferedReader reader = null;
			boolean foundDuplicate = false;
			
			
			File file = new File(Environment.getExternalStorageDirectory(), "/NP-History.txt");
			...
			try {
			    ...
			    reader = new BufferedReader(new FileReader(Environment.getExternalStorageDirectory() + "/NP-History.txt"));
		...
		public void updateResult(String result) {
			...
			//Store NumberPlate test ONLY to separate file for history
			
			if(result!=null && !TextUtils.isEmpty(result) && result!="" && !checkDuplicates(result)){
				String historyNumPlate = result +"\n";
				FileWriter f;
				try{
					f = new FileWriter(Environment.getExternalStorageDirectory() + "/NP-History.txt", true);
					//Log.e("DON", "BAT DAU GHI FILE 2222");
					f.write(historyNumPlate);
					f.flush();
					f.close();
				}catch (Exception e) {
